...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98]):
        ...
        if hf_key is not None:
            ...
            model_key = "/userhome/huangbin/models/stable-diffusion-2-1-base"
        # elif self.sd_version == '3.5':
        #     model_key = "stabilityai/stable-diffusion-2-base"
        elif self.sd_version == '1.5':
            model_key = "/userhome/huangbin/models/stable-diffusion-1-5"
        else:
        ...
        # Create model
        pipe = StableDiffusionPipeline.from_pretrained(model_key, torch_dtype=self.precision_t, safety_checker=None)
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, as_latent=False, grad_scale=1, sigma_detach=False, exp_iter_ratio=0, 
                   loss_weights=None, dmtet=False, dds=False,
        ...
        with torch.no_grad():
            ...
            latents_n = torch.cat(
                    [torch.cat([latents[index:index + 1]] * num) for index in range(latents.shape[0])]
                )

            text_embeddings_n = []
        
            for index in range(text_embeddings.shape[0]):
                text_embeddings_n.append(torch.cat([text_embeddings[index:index+1]] * num)) 
            text_embeddings_n = torch.cat(text_embeddings_n, dim=0)

            noise = torch.randn_like(latents_n)
            latents_noisy = self.scheduler.add_noise(latents_n, noise, t)

            latent_model_input = torch.cat(
                    [torch.cat([latents_noisy[index * num: (index + 1)* num]] * 2) for index in range(latents.shape[0])]
                )

            
            tt = torch.cat([t] * 2 * latents.shape[0] * num)
            
            
            noise_pred = self.unet(latent_model_input, tt, encoder_hidden_states=text_embeddings_n).sample

            noise_pred_chunks = noise_pred.chunk(latents.shape[0])
            preds = []
            for chunk in noise_pred_chunks:               
                
                noise_pred_uncond, noise_pred_pos = chunk.chunk(2)
                if dds: preds.append(torch.mean((noise_pred_pos - noise_pred_uncond), dim=0)[None,...])
                else: preds.append(noise_pred_uncond + guidance_scale * (noise_pred_pos - noise_pred_uncond))
        ...
        if dds: grad = grad_scale * w[:, None, None, None] * (noise_pred)
        else : grad = grad_scale * w[:, None, None, None] * (noise_pred - noise)
        grad = torch.nan_to_num(grad)

        
        if not dmtet and loss_weights is not None:
            for i in range(0, len(loss_weights)):
                grad[i] = grad[i] #* max(0.2, exp_iter_ratio)
