...
#from model.pipline_if import IFPipeline
...
class IF(nn.Module):
    def __init__(self, device, vram_O, t_range=[0.02, 0.98]):
        ...
        model_key = "/userhome/huangbin/models/IF-I-XL-v1.0"

        is_torch2 = torch.__version__[0] == '2'

        # Create model
        pipe = IFPipeline.from_pretrained(model_key, variant="fp16", torch_dtype=torch.float16, device_map='balanced')
        if not is_torch2:
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, grad_scale=1, exp_iter_ratio=0, loss_weights=[1]*10, dds=False, dmtet=False):
        ...
        with torch.no_grad():
            ...
            images_n = torch.cat(
                    [torch.cat([images[index:index + 1]] * num) for index in range(images.shape[0])]
                )
            ...
            noise = torch.randn_like(images_n)
            images_noisy = self.scheduler.add_noise(images_n, noise, t)

            # pred noise
            #model_input = torch.cat([images_noisy] * 2)
            
            model_input = torch.cat(
                    [torch.cat([images_noisy[index * num: (index + 1)* num]] * 2) for index in range(images.shape[0])]
                )
            
            model_input = self.scheduler.scale_model_input(model_input, torch.cat([t] * 2 * num))
            
            tt = torch.cat([t] * 2 * images.shape[0] * num)
            
            noise_pred = self.unet(model_input, tt, encoder_hidden_states=text_embeddings_n).sample

            noise_pred_chunks = noise_pred.chunk(images.shape[0])
            preds = []
            ...
            for chunk in noise_pred_chunks:
                ...
                if dds: preds.append(torch.mean((noise_pred_text - noise_pred_uncond), dim=0)[None,...])
                else: preds.append(noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond))
