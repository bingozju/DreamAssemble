...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        if self.dmtet:
            # load dmtet vertices
            tets = np.load('/userhome/huangbin/code/robust-text-3d-mask/tets/{}_tets.npz'.format(self.opt.tet_grid_size))
            ...
            for i in range(self.opt.part_nums):
                tets = np.load('/userhome/huangbin/code/robust-text-3d-mask/tets/{}_tets.npz'.format(self.opt.tet_grid_size))
                self.verts_parts.append(- torch.tensor(tets['vertices'], dtype=torch.float32, device='cuda') * 2) # covers [-1, 1]
                self.indices_parts.append(torch.tensor(tets['indices'], dtype=torch.long, device='cuda'))
                self.tet_scale_parts.append(torch.tensor([1, 1, 1], dtype=torch.float32, device='cuda'))
            ...
            for i in range(self.opt.part_nums):
                self.register_parameter(f'sdf_{i}', torch.nn.Parameter(torch.zeros_like(self.verts_parts[i][..., 0]), requires_grad=True))
                self.register_parameter(f'deform_{i}', torch.nn.Parameter(torch.zeros_like(self.verts_parts[i]), requires_grad=True))
    ...
    def density_blob(self, x, index=None):
        ...
        if index is not None:
            if self.opt.density_activation == 'exp':
                g = self.opt.blob_density * torch.exp(- d / (2 * self.opt.parts_blob_radius[index] ** 2))
            else:
                g = self.opt.blob_density * (1 - torch.sqrt(d) / self.opt.parts_blob_radius[index])
        else:
            if self.opt.density_activation == 'exp':
    ...
    def run(self, rays_o, rays_d, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, perturb=False, **kwargs):
        ...
        # density_outputs = self.density(xyzs)   
        ...
        ret = self(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), light_d.reshape(-1, 3), ratio=ambient_ratio, shading=shading)
        #ret = self(xyzs, dirs.reshape(-1, 3), light_d.reshape(-1, 3), ratio=ambient_ratio, shading=shading)
        
        #sigmas, rgbs, normals = ret['sigmas'], ret['rgbs'], ret['normals']
        rgbs, normals = ret['rgbs'], ret['normals']
        
        alphas_parts = []

       
        sigma_parts = density_outputs['sigma_parts']
        
        perm = torch.randperm(parts_num)
        ...
        for i in range(0, parts_num):
            ...
            temp = 1 - torch.exp(-deltas * sigma_parts_shuffled[..., i])
            for j in range(0, i):
                temp_j = torch.exp(-deltas * sigma_parts_shuffled[..., j])
                temp = temp * temp_j
                
            alphas_parts.append(temp)
       
        alphas_parts = torch.stack(alphas_parts)[inverse_perm] 
        
       
        # alphas_parts = 1 - torch.exp(-deltas * density_outputs['sigma_parts'].permute(2,0,1))
        
        alphas_parts = torch.concat((alphas_parts, (1 - torch.exp(-deltas * density_outputs['sigma'].squeeze(-1)))[None, ...]), dim=0)

        alphas_shifted = torch.cat([torch.ones_like(alphas_parts[..., :1]), 1 - alphas_parts + 1e-5], dim=-1) # [num + 1, N, T+t+1]
        weights = alphas_parts * torch.cumprod(alphas_shifted, dim=-1)[..., :-1] # [num + 1, N, T+t]
        
        # calculate weight_sum (mask)
        weights_sum = weights.sum(dim=-1).clamp(0, 1) # [num + 1, N]
        ...
        image = torch.sum(weights.unsqueeze(-1) * rgbs, dim=-2) #torch.cat([rgbs_parts, rgbs[None, ...]], dim=0), dim=-2) # [N, 3], in [0, 1]

        
        # image = torch.sum(weights.unsqueeze(-1) * torch.cat([rgbs_parts, rgbs[None, ...]], dim=0), dim=-2)
        image = image + (1 - weights_sum).unsqueeze(-1) * bg_color
        image = image.view(1 + parts_num, prefix[-1], 3)
        weights_sum = weights_sum.view(1 + parts_num, prefix[-1])
        ...
        if self.training:
            ...
            if (self.opt.lambda_2d_normal_smooth > 0 or self.opt.lambda_normal > 0) and normals is not None:
                normal_image = torch.sum(weights[-1].unsqueeze(-1) * (normals + 1) / 2, dim=-2) # [N, 3], in [0, 1]
                results['normal_image'] = normal_image
                
            #results['softmax'] = density_outputs['softmax']
        ...
        for i in range(parts_num):
            results[f'image_{i}'] = image[i]
            results[f'weights_sum_{i}'] = weights_sum[i]
            results[f'weights_{i}'] = weights[i]
    ...
    def init_tet(self, mesh=None):
        ...
        if mesh is not None:
            ...
            for i in range(self.opt.part_nums):
                ret = self.density(self.verts_parts[i]) # verts covers [-1, 1] now
                #sigma = ret['sigma'] * ret['mask'][:,i]
                sigma = ret['sigma_parts'][:,i]
                
                # init scale
                mask = sigma > density_thresh
                valid_verts = self.verts_parts[i][mask]
                self.tet_scale_parts[i] = valid_verts.abs().amax(dim=0) + 1e-1
                self.verts_parts[i] = self.verts_parts[i] * self.tet_scale_parts[i]

                # init sigma
                ret = self.density(self.verts_parts[i]) 
                #sigma = ret['sigma'] * ret['mask'][:,i] # new verts
                sigma = ret['sigma_parts'][:,i]
                
                sdf_i = getattr(self, f'sdf_{i}')
                sdf_i.data += (sigma - density_thresh).clamp(-1, 1)
                setattr(self, f'sdf_{i}', sdf_i)
    ...
    def run_dmtet(self, rays_o, rays_d, mvp, h, w, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, **kwargs):
        ...
        if mask.any():
            ret = self.density(xyzs[mask])
            masked_albedo = ret['albedo']
        ...
        for i in range(self.opt.part_nums):
            # get mesh
            sdf = getattr(self, f'sdf_{i}')
            deform = torch.tanh(getattr(self, f'deform_{i}')) / self.opt.tet_grid_size
            # verts, faces =        self.dmtet_model(self.verts + deform, sdf, self.indices)
            verts_, faces_ = self.dmtet_model_parts[i](self.verts_parts[i] + deform, sdf, self.indices_parts[i])
            faces_p.append(faces_)
            verts_p.append(verts_)
            # get normals
            i0, i1, i2 = faces_p[i][:, 0], faces_p[i][:, 1], faces_p[i][:, 2]
            v0, v1, v2 = verts_p[i][i0, :], verts_p[i][i1, :], verts_p[i][i2, :]

            faces_p[i] = faces_p[i].int()
            
            face_normals_p.append(torch.cross(v1 - v0, v2 - v0))
            face_normals_p[i] = safe_normalize(face_normals_p[i])
            
            vn = torch.zeros_like(verts_p[i])
            vn.scatter_add_(0, i0[:, None].repeat(1,3), face_normals_p[i])
            vn.scatter_add_(0, i1[:, None].repeat(1,3), face_normals_p[i])
            vn.scatter_add_(0, i2[:, None].repeat(1,3), face_normals_p[i])

            vn = torch.where(torch.sum(vn * vn, -1, keepdim=True) > 1e-20, vn, torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device=vn.device))

            # rasterization
            verts_clip_p = torch.bmm(F.pad(verts_p[i], pad=(0, 1), mode='constant', value=1.0).unsqueeze(0).repeat(mvp.shape[0], 1, 1), 
                                mvp.permute(0,2,1)).float()  # [B, N, 4]
            
            rast_p, rast_db = dr.rasterize(self.glctx, verts_clip_p, faces_p[i], (h, w))
            
            alpha = (rast_p[..., 3:] > 0).float()
            xyzs, _ = dr.interpolate(verts_p[i].unsqueeze(0), rast_p, faces_p[i]) # [B, H, W, 3]
            normal_p, _ = dr.interpolate(vn.unsqueeze(0).contiguous(), rast_p, faces_p[i])
            normal_p = safe_normalize(normal_p)

            xyzs = xyzs.view(-1, 3)
            mask = (rast_p[..., 3:] > 0).view(-1).detach()
            ...
            if mask.any():
                ...
                # 'albedo': albedo,
                # 'mask': mask,
                masked_albedo = ret['albedo'] #* ret['mask'][:, i:i+1]

                albedo[mask] = masked_albedo.float()

                # for i in range(self.opt.part_nums):
                #     albedo_parts[i][mask] = masked_albedo.float() * ret_mask[:,i:i+1]
                #     mask_parts[i][mask] = ret_mask[:,i]
                
            albedo = albedo.view(-1, h, w, 3)
            # for i in range(self.opt.part_nums):
            #     albedo_parts[i] = albedo_parts[i].view(-1, h, w, 3)
            #     mask_parts[i] = mask_parts[i].view(-1, h, w)
            ...
            if shading == 'albedo':
                ...
                lambertian = ambient_ratio + (1 - ambient_ratio)  * (normal_p * light_d).sum(-1).float().clamp(min=0)
                color = lambertian.unsqueeze(-1).repeat(1, 1, 1, 3)
            elif shading == 'normal':
                color = (normal_p + 1) / 2
            else: # 'lambertian'
            ...
            color = dr.antialias(color, rast_p, verts_clip_p, faces_p[i]).clamp(0, 1) # [B, H, W, 3]
            alpha = dr.antialias(alpha, rast_p, verts_clip_p, faces_p[i]).clamp(0, 1) # [B, H, W, 1]
            ...
            depth = rast_p[:, :, :, [2]] # [B, H, W]
            color = color + (1 - alpha) * bg_color
            
            results[f'depth_{i}'] = depth
            results[f'image_{i}'] = color
            results[f'weights_sum_{i}'] = alpha.squeeze(-1)
            # results[f'weights_{i}'] = weights[i]
        ...
        if self.training:
            if self.opt.lambda_mesh_normal > 0:
                for i in range(self.opt.part_nums):
                    normal_loss_p += normal_consistency(face_normals_p[i], faces_p[i])
                
            if self.opt.lambda_mesh_laplacian > 0:
                for i in range(self.opt.part_nums):
                    lap_loss_p += laplacian_smooth_loss(verts_p[i], faces_p[i])
        ...
        if self.training:
            if self.opt.lambda_mesh_normal > 0:
                results['normal_loss'] = normal_consistency(face_normals, faces) + normal_loss_p
           
            if self.opt.lambda_mesh_laplacian > 0:
                results['lap_loss'] = laplacian_smooth_loss(verts, faces) + lap_loss_p
    ...
    def render(self, rays_o, rays_d, mvp, h, w, staged=False, max_ray_batch=4096, **kwargs):
        ...
        if self.dmtet:
            ...
            if staged:
                ...
                image_parts = []
                for i in range(0, self.opt.part_nums):
                    image_parts.append(torch.empty((B, N, 3), device=device))
                ...
                for b in range(B):
                    ...
                    while head < N:
                        ...
                        for i in range(0, self.opt.part_nums):
                            image_parts[i][b:b+1, head:tail] = results_[f'image_{i}']
                        head += max_ray_batch
                ...
                for i in range(0, self.opt.part_nums):
                    results[f'image_{i}'] = image_parts[i]
